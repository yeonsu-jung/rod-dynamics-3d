...
targetFrame = frames(1);  % last frame
fprintf('Analyzing frame: %d\n', targetFrame);

F = T(T.(frameCol) == targetFrame, :);

% Use node NAMES (strings) so MATLAB doesn't require positive integer indices
s = string(F.(rodICol));
t = string(F.(rodJCol));

% Drop missing/NaN endpoints
mask = ~ismissing(s) & ~ismissing(t) & s ~= "NaN" & t ~= "NaN";
s = s(mask);
t = t(mask);

G = graph(s, t);     % undirected by default with edge list
G = simplify(G);     % removes self-loops + parallel edges

deg = degree(G);

figure('Name', sprintf('Contact Network - Frame %d', targetFrame), 'Color','w');
p = plot(G);
p.NodeLabel = {}; % hide labels (usually too dense)
p.NodeCData = deg;
