...
def main() -> None:
    ...
    ap.add_argument("--bundle-all", action="store_true",
                    help="Bundle all filtered entries into one SLURM Array job.")
    ...
    if args.bundle_all:
        ...
        cmds_file = run_dir / "array_commands.txt"
...
#SBATCH -o {run_dir.absolute()}/output_%A_%a.out
#SBATCH -e {run_dir.absolute()}/errors_%A_%a.err
#SBATCH --mail-type={slurm.mail_type}
{f"#SBATCH --mail-user={slurm.mail_user}" if slurm.mail_user else ""}
#SBATCH --job-name={jname}_Array
...
echo "Job complete."
"""
        master_sbatch_path = run_dir / "Master_Sbatch.sh"
        master_sbatch_path.write_text(master_sb)

        print(f"Array dir: {run_dir}")
        print(f"  Tasks: {len(array_commands)}")

        if not args.dry_run:
            if args.local:
                print("Running sequentially locally...")
                for cmd in array_commands:
                     subprocess.run(cmd, shell=True)
            else:
                r = subprocess.run(["sbatch", "Master_Sbatch.sh"], cwd=run_dir)
                if r.returncode != 0:
                    print("WARNING: sbatch failed.")
                else:
                    print("Submitted array job.")
        else:
            print(f"Dry run: {master_sbatch_path}")
        return
