...
for ar in "${ars[@]}"; do
  run_dataset "$ar" \
    "results/reptation_ar${ar}_const_vn0p1_vt0_va0p1_w0_gapdiam_mugeom10_first" \
    --fixed-reptation --fixed-vn 0.1 --fixed-vt 0.0 --fixed-va 0.1 --fixed-w 0.0

  run_dataset "$ar" \
    "results/reptation_ar${ar}_const_vn0p1_vt0_va0p1_w0p2_gapdiam_mugeom10_first" \
    --fixed-reptation --fixed-vn 0.1 --fixed-vt 0.0 --fixed-va 0.1 --fixed-w 0.2

  run_dataset "$ar" \
    "results/reptation_ar${ar}_gauss_svn0p1_svt0_sva0_sw0p2_gapdiam_mugeom10_first" \
    --init-mode gaussian-axial-transverse --sigma-vn 0.1 --sigma-vt 0.0 --sigma-va 0.0 --sigma-w-reptation 0.2

  echo "=== completed all init families for AR=$ar ==="
done

echo "=== all requested reptation sweeps completed ==="