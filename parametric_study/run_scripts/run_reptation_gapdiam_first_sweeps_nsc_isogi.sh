...
for ar in "${ars[@]}"; do
  run_dataset "$ar" \
    "results/reptation_ar${ar}_nsc_isogi_sv0p1_sw0p2_gapdiam_mugeom10_first" \
    --init-mode gaussian-isotropic --sigma-v 0.1 --sigma-w 0.2

  echo "=== completed requested NSC isotropic Gaussian family for AR=$ar ==="
done

echo "=== all requested NSC isotropic Gaussian sweeps completed ==="