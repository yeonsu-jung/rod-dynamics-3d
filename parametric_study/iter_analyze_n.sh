...
SCRIPT="parametric_study/submit_analysis.sh"

echo "Scanning $RUNS_BASE for relax2nd_*_sweep..."

for dir in "$RUNS_BASE"/relax2nd_*_sweep; do
    if [ -d "$dir" ]; then
        dirname=$(basename "$dir") # e.g., relax2nd_N500_sweep
        
        # Extract N:
        # 1. Remove everything up to "N" (e.g. "relax2nd_N")
        tmp=${dirname#*_N}
        ...
        # Filter for N=1000 per user request
        if [ "$N" -ne 1000 ]; then
            echo "Skipping N=$N"
            continue
        fi
