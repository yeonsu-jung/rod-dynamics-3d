...
def main() -> None:
    ...
    for x_path, ar in jobs:
        ...
        run_name = safe_name(f"{timestamp}_{seed_folder}_AR{ar}")
        ...
        # Write scene.json (copy of default_entangled.json)
        (run_dir / "scene.json").write_text(json.dumps(base_scene, indent=2))

        # Command
        sim_parts: List[str] = [
            "time ./rigidbody_viewer_3d",
            "--headless",
            "--scene scene.json",
            f"--init-csv {shlex_quote(str(x_path))}",
            "--output output.csv",
        ...
        if int(args.perrod_stride) > 0:
            ...
            # If perrod is enabled, we need to make sure we enable the perrod logging in the binary
            # The binary uses --perrod or --per-rod to enable it.
            # Using --perrod-stride typically implies enabling it, but let's be explicit if path is needed
            # The binary default path is perrod.csv if not provided. 
            # We'll just pass the stride and let the binary handle enabling it implicitly if implemented, 
            # or explicitly enable it.
            # Looking at main.cpp: "if (cliPerrodStride > 0) a.setPerRodStride(cliPerrodStride);"
            # and "if (!perRodPath.empty()) a.enablePerRod(perRodPath, perRodMaxFrames);"
            # We might need to pass --perrod perrod.csv too to ensure it's enabled.
        ...
        if not args.no_entanglement:
            ...
            # NOTE: main.cpp historically accepted only the long-form flags.
            # We pass both long and short aliases; whichever is supported will
            # be used.
...
"""
    ...
    if args.dry_run:
        print(f"Dry-run complete: created {len(jobs)} run folders under {runs_root}")
    else:
        print(f"Submitted {submitted} jobs under {runs_root}")
