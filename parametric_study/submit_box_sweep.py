...
def main():
    ...
    # subfolders = [d for d in subfolders if any(d.name.startswith(f"ar{r}_") for r in ["30", "100", "300"])]
    subfolders = [d for d in subfolders if any(d.name.startswith(f"ar{r}_") for r in ["200"])]
    
    
    normalized_densities = [0.3, 1, 3, 10]
    ...
    for subdir in subfolders:
        ...
        for norm_density in normalized_densities:
            # Calculate N = box_size^3 / (L^2 * d)
            num_rods = int(norm_density * box_size**3 / (L**2 * d))
            
            print(f"Processing {subdir.name}: dens={norm_density}, L={L}, d={d}, box={box_size} -> N={num_rods}")
    
            for fSigma in f_sigmas:
                # Create run dir
                run_name = f"{timestamp}_{subdir.name}_dens{norm_density}_fSig{fSigma:.1e}"
                ...
                scene['physics']['lin_damp'] = 1.0
                scene['physics']['ang_damp'] = 1.0
                ...
                sim_cmd = (
                    ...
                    f"--output-max {max_frames} "
                    "--network network.csv "
                    f"--network-stride {stride} "
                    f"--network-max {max_frames} "
                    "--perrod perrod.csv "
                    f"--perrod-stride {stride} "
                    f"--perrod-max {max_frames} "
                    f"--csv-stride {stride} "  # Keep profile CSV on same stride
                    f"--snap-stride {stride} "
                    f"--snap-frames {max_frames} "
                    f"--steps {steps} "
...
set -euo pipefail
export OMP_NUM_THREADS=${{SLURM_CPUS_PER_TASK:-{SLURM['cpus']}}}
{SLURM['module_line']}
