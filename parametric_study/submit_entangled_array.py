...
def main() -> None:
    ...
    for x_path, ar in jobs:
        for friction in friction_values:
            ...
            sim_cmd_full = f"cd {shlex_quote(str(run_dir.absolute()))} && {sim_cmd}"
            array_commands.append(sim_cmd_full)

    if not array_commands:
        print(f"No new jobs to run. Skipped {skipped}.")
        return

    cmds_file = runs_root / "array_commands.txt"
...
#SBATCH -o {runs_root.absolute()}/output_%A_%a.out
#SBATCH -e {runs_root.absolute()}/errors_%A_%a.err
#SBATCH --mail-type={slurm.mail_type}
{f"#SBATCH --mail-user={slurm.mail_user}" if slurm.mail_user else ""}
#SBATCH --job-name={safe_name(job_name)}_Array

set -euo pipefail
{slurm.module_line}

CMD=$(sed -n "${{SLURM_ARRAY_TASK_ID}}p" {cmds_file.absolute()})
...
echo "Job complete."
"""
    master_sbatch_path = runs_root / "Master_Sbatch.sh"
    master_sbatch_path.write_text(master_sb)

    if not args.dry_run:
        print(f"Submitting master array job for N={args.n_rods} ({len(array_commands)} tasks)...")
        result = subprocess.run(["sbatch", "Master_Sbatch.sh"], cwd=runs_root)
        if result.returncode != 0:
            print("  WARNING: master sbatch failed.")
    else:
        print(f"Dry run: Created {master_sbatch_path} with {len(array_commands)} tasks.")
        
    print(f"Skipped {skipped} runs (already done).")
