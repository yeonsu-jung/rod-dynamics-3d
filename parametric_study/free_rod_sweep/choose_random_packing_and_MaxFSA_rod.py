...
def resolve_free_rod_id(args: argparse.Namespace, packing: RelaxedPacking) -> int:
    if args.free_rod_id is None:
        return random.randrange(packing.n_rods)
    if not 0 <= args.free_rod_id < packing.n_rods:
        raise ValueError(
            f"free_rod_id={args.free_rod_id} is out of range for packing with "
            f"{packing.n_rods} rods; expected 0 <= id < {packing.n_rods}"
        )
    return args.free_rod_id


def build_command(args: argparse.Namespace, init_csv_path: Path, free_rod_id: int) -> list[str]:
    cmd = [
        str(BINARY_PATH),
        "--scene",
        str(SCENE_PATH),
...
def main() -> int:
    ...
    # free_rod_id = resolve_free_rod_id(args, packing) # this is a random choice
    # print(f"Selected free rod: {free_rod_id}")
    
    # choose test rod - having MaxFSA (MaxFTA, later)
    from look_up_extreme_rods import look_up_extreme_rod
    rod_index, value = look_up_extreme_rod(packing.n_rods,packing.aspect_ratio,packing.seed_triplet,"MaxFSA")
    free_rod_id = resolve_free_rod_id(args, packing) if args.free_rod_id is not None else rod_index
    print(f"Selected extreme rod: {rod_index} (MaxFSA={value})")
    print(f"Using free rod: {free_rod_id}")
    ...
    cmd = build_command(args, init_csv_path, free_rod_id)
