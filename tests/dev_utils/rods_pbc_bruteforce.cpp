...
// Minimum-image displacement for a component given box length L (sim's
// convention)
...
static inline double dist2_seg_pbc(const Vec3 &p0, const Vec3 &p1,
                                   const Vec3 &q0, const Vec3 &q1, double L) {
...
vector<RodAngles>
generate_rods_bruteforce_pbc(int N, double C, double rod_length,
                             double rod_diameter, uint64_t seed,
  ...
  for (int i = 0; i < N; ++i) {
    bool placed = false;
    for (int attempt = 0; attempt < max_attempts_per_rod && !placed;
         ++attempt) {
      double cx = U(-C, C);
      double cy = U(-C, C);
      double cz = U(-C, C);
    ...
    if (!placed) {
      cerr << "Failed to place rod " << i << " without overlap after "
           << max_attempts_per_rod << " attempts\n";
      break;
    }
  }
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (a == "--help" || a == "-h") {
      cout << "Usage: " << argv[0] << " [options]\n"
           << "Options:\n"
           << "  --C <val>           Half-width of the box (box size = 2*C) "
              "[default: "
           << C << "]\n"
           << "  --box_size <val>    Full box size (overrides --C) [default: "
           << (2 * C) << "]\n"
           << "  --rod_length <val>  Length of rods [default: " << rod_length
           << "]\n"
           << "  --alpha <val>       Aspect ratio (length/diameter) [default: "
           << alpha << "]\n"
           << "  --N <val>           Number of rods [default: " << N << "]\n"
           << "  --seed <val>        RNG seed [default: " << seed << "]\n"
           << "  --output <file>     Output CSV file [default: " << output_path
           << "]\n";
      return 0;
    }
    ...
    if (a == "--C")
      ...
      C = 0.5 * stod(need("--box_size"));
    else if (a == "--rod_length")
      rod_length = stod(need("--rod_length"));
    else if (a == "--alpha")
      alpha = stod(need("--alpha"));
    else if (a == "--N")
      N = stoi(need("--N"));
    else if (a == "--seed")
      seed = stoull(need("--seed"));
    else if (a == "--output" || a == "-o")
      output_path = need("--output");
    else {
      cerr << "Unknown arg: " << a << "\n";
      return 2;
    }
  ...
  double box_size = 2.0 * C;
  double box_vol = box_size * box_size * box_size;
  double factor = (double)N / box_vol * rod_diam * rod_length * rod_length;
  cout << "Dimensionless factor (N/V * D * L^2): " << factor << "\n";

  auto t0 = chrono::high_resolution_clock::now();
  vector<RodAngles> rods =
      generate_rods_bruteforce_pbc(N, C, rod_length, rod_diam, seed);
  ...
  cout << "All-pairs PBC min gap: " << minGap << " (i=" << minI
       << ", j=" << minJ << ")\n";

  if (minGap < 0.0) {
    cerr << "FATAL: Generated configuration has overlap (minGap=" << minGap
         << ")\n";
    return 1;
  }
