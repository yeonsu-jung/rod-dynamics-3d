...
// External globals from solver/integrator (these will eventually be removed)
extern float g_lin_damp;
extern float g_ang_damp;
extern float g_w_max;
extern bool g_pbc_enabled;
extern glm::vec3 g_pbc_min;
extern glm::vec3 g_pbc_max;
extern void resetFrameImpulseAccumulators();
...
World::World() {
    genRandomForce.seed(std::random_device{}());
}

void World::setPBC(bool enabled, const glm::vec3& min, const glm::vec3& max, float cs) {
    usePBC = enabled;
    pbcMin = min;
    pbcMax = max;
    cellSize = cs;
...
void World::setRandomForce(bool enabled, float fSig, float tau, unsigned int seed) {
...
void World::setSleepThresholds(float linThresh, float angThresh, float timeThresh) {
...
void World::setFloor(const RigidBody& floor) {
    floorRB = floor;
}
...
void World::integrateRods() {
    ...
    if (softContactEnabled) {
        ...
        // Hard contact uses symplectic Euler
        parallel_for(0, rods.size(), [&](size_t i) {
...
void World::wrapPBC() {
    ...
    for (auto& rb : rods) {
        for (int k = 0; k < 3; ++k) {
            float& coord = rb.x[k];
            if (coord < pbcMin[k]) coord += size[k];
            else if (coord >= pbcMax[k]) coord -= size[k];
        }
    }
...
glm::ivec3 World::gridDims(const glm::vec3& bmin, const glm::vec3& bmax, float cs) {
    glm::vec3 size = bmax - bmin;
    return glm::max(glm::ivec3(1), glm::ivec3(glm::floor(size / cs)));
}

glm::ivec3 World::cellIndex(const glm::vec3& p, const glm::vec3& bmin, const glm::vec3& bmax, const glm::ivec3& n) {
    glm::vec3 size = bmax - bmin;
...
void World::wrapPos(glm::vec3& p, const glm::vec3& bmin, const glm::vec3& bmax) {
    const glm::vec3 size = bmax - bmin;
    for (int k = 0; k < 3; ++k) {
        if (p[k] < bmin[k]) p[k] += size[k];
        else if (p[k] >= bmax[k]) p[k] -= size[k];
    }
}

void World::performCollisionResolution() {
    // This is the large collision detection + resolution block from App::physicsStep
    // For now, we'll implement a simplified version and expand it in the next step
