...
static inline void wrap_position(glm::vec3 &p, const glm::vec3 &bmin,
                                 const glm::vec3 &bmax) {
...
void integrate(RigidBody &body, const glm::vec3 &gravity, float deltaTime) {
  ...
  // We previously attempted a single-step Velocity Verlet half-step update, but
  // because forces are only computed once per frame (prior to this call) we
  // never performed the second half velocity update, causing artificial energy
  // decay. We revert to the classic symplectic Euler: first apply full
  // acceleration to velocity, then advance position. This is stable for our
  // small dt (1/12000) and preserves energy better with penalty forces.
...
void integrateHalfPos(RigidBody &body, const glm::vec3 &gravity,
                      float deltaTime) {
  if (body.invMass <= 0.0f)
    return;
  const glm::vec3 acc_lin = (body.f / body.mass) + gravity;
...
void integrateSecondHalf(RigidBody &body, const glm::vec3 &gravity,
                         float deltaTime) {
  if (body.invMass <= 0.0f)
    return;
  const glm::vec3 acc_lin = (body.f / body.mass) + gravity;
